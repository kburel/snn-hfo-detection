...
Network_parameters = sio.loadmat(parameters_path + 'Network_parameters.mat')
...
Interval = sio.loadmat(data_path + 'P%s/P%sI%s'%(patient,patient,current_interval) + '.mat')

num_channels = Interval['chb'].shape[0]
for ch in range(num_channels):
    
    print('Running test for Patient %s, interval %s and channel %s' %(patient,current_interval,ch))
    ...
    Input_Hidden_layer.I_tau = getTauCurrent(Network_parameters['synapse_taus'][0]*1e-3, True) * amp
    Input_Hidden_layer.baseweight = 1 * pamp

    #-----------% SNN Monitors %-----------#
    Spike_Monitor_Hidden = SpikeMonitor(Hidden_layer)
    Hidden_Monitor = StateMonitor(Hidden_layer, variables=['Iin'], record= [0], name='Test_Hidden_Monitor')
    ...
    HFO_detection = detect_HFO(trial_duration = duration, 
                               spike_monitor = (Spike_Monitor_Hidden.t/second), 
    ...
    print('Rate of HFO (event/min)', np.around((detected_HFO/duration)*60, decimals = 2))   
    print(' ')
